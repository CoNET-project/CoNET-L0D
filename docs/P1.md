...
Carry overlay IPv4 (geth / beacon TCP toward a peer vIP) as a wallet-to-wallet byte stream. Prefer **application duplex**: offer on the long-lived Chat SSE; accept / reject / frames on **session listen** SSEs. `duplex_reject` or a missing `duplex_accept` keeps **P1 gossip**. SI does **not** implement `duplex_*`. Do not invent `p2p_stream_*` or `listenKind: "l1p2p"`. Never send `command: "mining"` with `listenKind: "duplex"`.
...
| `duplex_accept` (key echo + responder session listen) | Initiator **session listen user PGP** | Healthy entry **A ≠ B**. Lands on the initiator session Chat SSE |
| `duplex_reject` | Initiator **session listen user PGP** | Healthy entry **A ≠ B**. Keep P1 gossip |
| App-layer session listen | **Own B route PGP** | Existing `mining` + `listenKind: "chat"` via **C ≠ B**. Crate MVP: per-port channel EOA. Not an SI `duplex_server_listen` |
| Overlay IPv4 (duplex data) | AES-256-GCM of `L0D1` \|\| IPv4 inside user-PGP `duplex_frame` to the peer **session listen user PGP**, then mailbox wrap | Entry **A ≠ B**. Same `{ "data" }` as P1 gossip |
| P1 gossip fallback | Peer **user PGP**, then mailbox work (`NoPush`) to **B route PGP** | Same `{ "data" }` body — never an HTTP field |
| Chat-pool listen (channel + session in MVP) | **B route PGP**, `command: mining` + `listenKind: "chat"` on a **dedicated routing EOA** (optional `[[l0.channels]]`: one EOA + SSE per overlay port 8400 / 4200 / 4300) | Healthy entry **C ≠ B** |
...
| Listen HTTP+SSE worker (POST `{ data }` to C, read stream, `apply_inbound_armor`) | **In-crate**; mock-tested; EIP-191 `mining`; unused when `[l0]` is off. Optional `[[l0.channels]]` spawns one worker per overlay port |
...
| Application duplex | **Not SI.** Offer / accept / frame ride Chat gossip; host listen is existing Chat SSE. Spec: GitBook `l0/duplex-forward` |
