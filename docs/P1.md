...
**Revision:** 2026-08-18 (optional `[[l0.channels]]` per-port routing EOA + listen SSE; overlay IPv4 batch + POST 32/512; Prysm-bound follow-the-chain; ~17:28Z authorized `.45` `restart-beacon` restored overlay DHT TCP / `Processing blocks` while EL stayed `0x0`; separate lab UDP/DHT comms in [P2.md](./P2.md) does not close this gate)
...
| Listen | **B route PGP**, `command: mining` + `listenKind: "chat"` on a **dedicated routing EOA** (optional `[[l0.channels]]`: one EOA + SSE per overlay port 8400 / 4200 / 4300) | Healthy entry **C ≠ B** |
