...
**Revision:** 2026-08-17 (milestone eval 23:30Z)
...
| Listen HTTP+SSE worker (POST `{ data }` to C, read SSE, `apply_inbound_armor`) | **In-crate**; mock-tested; EIP-191 `mining`; unused when `[l0]` is off |
| Production SI listen / live mailbox SSE | **Not** opened (`[l0]` stays off; do not POST production SI) |
...
## This-stage evaluation (2026-08-17 23:30Z)
...
| P1 listen HTTP+SSE + EIP-191 SI wrap in-crate (`cargo test` 47; mock SSE) | **Accepted** |
