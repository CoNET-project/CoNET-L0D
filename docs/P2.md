...
**Revision:** 2026-08-18 (lab overlay UDP accepted; live Prysm discv5 from L0_ONLY `.45` to the `.98` DHT server over L0 accepted; drop recovery = flush ghost conntrack first; authorized `.45` `restart-beacon` after dial backoff restores overlay libp2p TCP; `ss` public `:4200` is DNAT dest, not a leak; not a production discv5 product)
...
This page is a **lab communications experiment**. It does **not** close the [P1](./P1.md) follow-the-chain gate. It is **not** a production discv5 / hybrid-P2P product.
...
| `scripts/overlay-dht-steer.sh` | L0_ONLY `.45` | DNAT hub `:4300`/`:4200` → `100.64.0.6`; MARK **after** DNAT (mangle POSTROUTING dest VIP); table 6300 has overlay CIDR **and** hub public `/32` via TUN; `apply` flushes stale hub `:4300`/`:4200` conntrack (**first** reconnect step; does not restart geth/beacon) |
| `scripts/start-shared-beacon-98.sh restart-beacon` | `.98` | `--p2p-static-id` DHT hub. Does **not** restart geth |
