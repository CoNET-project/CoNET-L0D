...
Overlay beacon identity is **pinned**: `06_restart_node66.sh` `start_beacon` passes `--p2p-static-id` (systemd `conet-node66.service` `ExecStart` → `start`, and `restart-beacon`). Keys live in `network/node-0/consensus/beacondata/network-keys`. Current overlay `--peer` is `/ip4/100.64.0.7/tcp/4200/p2p/16Uiu2HAmDJCHuVkXtkPrrL8YykQ9gFZnQkR9Q6WjZZUrmueohPfd`. **Do not** `systemctl restart conet-node66.service` (oneshot would bounce geth+validator). `restart-beacon-clean` wipes `beacondata` and mints a new peer_id.
